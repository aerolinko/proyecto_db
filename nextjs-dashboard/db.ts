...
// =====================================================
// FUNCIONES PARA AFILIACIÓN DE CLIENTES
// =====================================================

// Crear cliente natural con afiliación completa
export async function crearClienteNaturalAfiliado(
  rif: string,
  cedula: number,
  primerNombre: string,
  ...
  try {
    console.log("=== crearClienteNaturalAfiliado ===")
    
    const result = await sql`
      CALL crear_cliente_natural_afiliado(
        ${rif}, ${cedula}, ${primerNombre}, ${segundoNombre},
        ${primerApellido}, ${segundoApellido}, ${direccion},
        ${fkLugar}, ${codigoTelefono}, ${numeroTelefono},
        ${usuarioEmail}, ${dominioEmail}, ${password}
      )
    `;
    
    console.log("Cliente natural creado exitosamente")
    return { success: true, message: "Cliente natural creado exitosamente" }
  } catch (error) {
    console.error("Error en crearClienteNaturalAfiliado:", error)
    throw error
  }
}

// Obtener información completa del cliente natural
export async function obtenerClienteNaturalCompleto(clienteId: number) {
  try {
    console.log("=== obtenerClienteNaturalCompleto ===")
    
    const result = await sql`SELECT * FROM obtener_cliente_natural_completo(${clienteId})`
    
    console.log(`Cliente encontrado: ${result.length} registros`)
    return result[0] || null
  } catch (error) {
    console.error("Error en obtenerClienteNaturalCompleto:", error)
    throw error
  }
}

// Verificar si cliente existe por cédula o RIF
export async function verificarClienteExistente(
  cedula?: number,
  rif?: string
) {
  try {
    console.log("=== verificarClienteExistente ===")
    console.log("Cédula a verificar:", cedula)
    console.log("RIF a verificar:", rif)
    ...
    // Verificar por cédula en cliente_natural
    if (cedula) {
      const clienteNatural = await sql`SELECT cliente_id FROM cliente_natural WHERE cedula = ${cedula}`
      if (clienteNatural.length > 0) {
        existe = true
        tipoCliente = 'natural'
        clienteId = clienteNatural[0].cliente_id
        console.log("Cliente natural encontrado por cédula:", clienteId)
      }
    }
    
    // Si no existe por cédula, verificar por RIF en cliente_natural
    if (!existe && rif) {
      const clienteNaturalRif = await sql`SELECT cliente_id FROM cliente_natural WHERE rif = ${rif}`
      if (clienteNaturalRif.length > 0) {
        existe = true
        tipoCliente = 'natural'
        clienteId = clienteNaturalRif[0].cliente_id
        console.log("Cliente natural encontrado por RIF:", clienteId)
      }
    }
    
    // Si no existe en natural, verificar en cliente_juridico
    if (!existe && rif) {
      const clienteJuridico = await sql`SELECT cliente_id FROM cliente_juridico WHERE rif = ${rif}`
      if (clienteJuridico.length > 0) {
        existe = true
        tipoCliente = 'juridico'
        clienteId = clienteJuridico[0].cliente_id
        console.log("Cliente jurídico encontrado por RIF:", clienteId)
      }
    }
    
    console.log("Resultado final:", { existe, tipoCliente, clienteId })
    
    return { existe, tipo_cliente: tipoCliente, cliente_id: clienteId }
  } catch (error) {
    console.error("Error en verificarClienteExistente:", error)
    throw error
  }
}

// Generar código QR para cliente
export async function generarCodigoQRCliente(clienteId: number, tipoCliente: string) {
  try {
    console.log("=== generarCodigoQRCliente ===")
    
    const result = await sql`SELECT * FROM generar_codigo_qr_cliente(${clienteId}, ${tipoCliente})`
    
    console.log(`Código QR generado: ${result.length} registros`)
    return result[0] || null
  } catch (error) {
    console.error("Error en generarCodigoQRCliente:", error)
    throw error
  }
}
...
export async function actualizarPuntosCliente(
  ...
  try {
    console.log("=== actualizarPuntosCliente ===")
    
    await sql`CALL actualizar_puntos_cliente(${clienteId}, ${tipoCliente}, ${puntosASumar})`
    
    console.log("Puntos actualizados exitosamente")
    return { success: true, message: "Puntos actualizados exitosamente" }
  } catch (error) {
...
// =====================================================
// FUNCIÓN PRINCIPAL PARA AFILIACIÓN DESDE LANDING PAGE
// =====================================================

export async function afiliarClienteDesdeLanding(
  datosCliente: {
    rif: string;
    cedula: number;
    primerNombre: string;
  ...
  try {
    console.log("=== afiliarClienteDesdeLanding ===")
    ...
    if (clienteExistente.existe) {
      throw new Error(`Cliente ya existe con ${clienteExistente.tipo_cliente === 'natural' ? 'cédula' : 'RIF'}: ${datosCliente.cedula || datosCliente.rif}`)
    }
    
    // Separar email en usuario y dominio con formato correcto
    const [usuarioEmail, dominioEmail] = datosCliente.email.split('@')
    
    if (!dominioEmail) {
      throw new Error("Formato de email inválido")
    }
    
    // Formatear dominio con @ al inicio para cumplir la restricción check_dominio_correo_electronico
    const dominioFormateado = `@${dominioEmail}`
    
    // Crear cliente natural
    await crearClienteNaturalAfiliado(
      datosCliente.rif,
      datosCliente.cedula,
      ...
      datosCliente.fkLugar,
      datosCliente.codigoTelefono,
    ...
    // Obtener información del cliente creado por cédula
    const clientePorCedula = await getClienteNaturalPorCedula(datosCliente.cedula)
    
    if (!clientePorCedula) {
      throw new Error("Error al obtener información del cliente creado")
    }
    
    const clienteCreado = await obtenerClienteNaturalCompleto(clientePorCedula.cliente_id)
    
    // Generar código QR
    const codigoQR = await generarCodigoQRCliente(clienteCreado.cliente_id, 'natural')
    
    return {
      success: true,
      message: "Cliente afiliado exitosamente",
      cliente: clienteCreado,
      carnet: {
        numero: clienteCreado.carnet_numero,
        nombre: clienteCreado.nombre_completo,
        identificacion: clienteCreado.cedula,
        codigoQR: codigoQR.codigo_qr
      }
    }
  } catch (error) {
    console.error("Error en afiliarClienteDesdeLanding:", error)
    throw error
  }
}

// =====================================================
// FUNCIONES AUXILIARES PARA GESTIÓN DE CLIENTES
// =====================================================

// Obtener cliente natural por cédula
export async function getClienteNaturalPorCedula(cedula: number) {
  try {
    console.log("=== getClienteNaturalPorCedula ===")
    
    const result = await sql`SELECT * FROM obtener_cliente_natural_por_cedula(${cedula})`
    
    console.log(`Cliente natural encontrado: ${result.length} registros`)
    return result[0] || null
  } catch (error) {
    console.error("Error en getClienteNaturalPorCedula:", error)
    throw error
  }
}

// Obtener cliente jurídico por RIF
export async function getClienteJuridicoPorRif(rif: string) {
  try {
    console.log("=== getClienteJuridicoPorRif ===")
    
    const result = await sql`SELECT * FROM obtener_cliente_juridico_por_rif(${rif})`
    
    console.log(`Cliente jurídico encontrado: ${result.length} registros`)
    return result[0] || null
  } catch (error) {
    console.error("Error en getClienteJuridicoPorRif:", error)
    throw error
  }
}

// Obtener información de contacto del cliente
export async function getContactoCliente(clienteId: number, tipoCliente: string) {
  try {
    console.log("=== getContactoCliente ===")
    
    const result = await sql`SELECT * FROM obtener_contacto_cliente(${clienteId}, ${tipoCliente})`
    
    console.log(`Contacto encontrado: ${result.length} registros`)
    return result[0] || null
  } catch (error) {
    console.error("Error en getContactoCliente:", error)
    throw error
  }
}

// Obtener métodos de pago del cliente
export async function getMetodosPagoCliente(clienteId: number, tipoCliente: string) {
  try {
    console.log("=== getMetodosPagoCliente ===")
    
    const result = await sql`SELECT * FROM buscarMetodosDePagoCliente(${tipoCliente}, ${clienteId})`
    
    console.log(`Métodos de pago encontrados: ${result.length} registros`)
    return result
  } catch (error) {
    console.error("Error en getMetodosPagoCliente:", error)
    throw error
  }
}

// =====================================================
// FUNCIONES PARA GESTIÓN DE CARNÉT DIGITAL
// =====================================================

// Generar carnet digital completo
export async function generarCarnetDigital(clienteId: number, tipoCliente: string) {
  try {
    console.log("=== generarCarnetDigital ===")
    
    const result = await sql`SELECT * FROM generar_carnet_digital(${clienteId}, ${tipoCliente})`
    
    if (result.length === 0) {
      throw new Error("Cliente no encontrado")
    }
    
    const carnet = result[0]
    
    console.log("Carnet digital generado exitosamente")
    return carnet
  } catch (error) {
    console.error("Error en generarCarnetDigital:", error)
    throw error
  }
}

// Validar carnet digital
export async function validarCarnetDigital(numeroCarnet: string, codigoQR: string) {
  try {
    console.log("=== validarCarnetDigital ===")
    
    const result = await sql`SELECT * FROM validar_carnet_digital(${numeroCarnet}, ${codigoQR})`
    ...
    console.error("Error en validarCarnetDigital:", error)
    throw error
  }
}