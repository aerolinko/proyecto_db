...
-- Ejemplo: Obtener actividades de un evento específico
-- SELECT * FROM obtener_actividades_evento(1);

-- Ejemplo: Crear una nueva actividad
-- SELECT crear_actividad(1, 'Degustación de cervezas', '2024-01-15', '14:00', '16:00');

-- Ejemplo: Obtener todas las actividades
-- SELECT * FROM obtener_todas_actividades();

-- Ejemplo: Ver actividades de hoy
-- SELECT * FROM vista_actividades_hoy;

-- Ejemplo: Ver todas las actividades con información completa
-- SELECT * FROM vista_actividades_completas;

-- 6. CONSULTAS ÚTILES ADICIONALES
-- =====================================================

-- Obtener actividades por rango de fechas
CREATE OR REPLACE FUNCTION obtener_actividades_por_fechas(fecha_inicio DATE, fecha_fin DATE)
RETURNS TABLE (
    premiacion_id INTEGER,
    nombre VARCHAR(50),
    fecha DATE,
...
-- Obtener estadísticas de actividades por evento
CREATE OR REPLACE FUNCTION estadisticas_actividades_evento(evento_id_param INTEGER)
RETURNS TABLE (
    total_actividades INTEGER,
    actividades_hoy INTEGER,
    actividades_futuras INTEGER,
    actividades_pasadas INTEGER
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        COUNT(*)::INTEGER as total_actividades,
        COUNT(CASE WHEN p.fecha = CURRENT_DATE THEN 1 END)::INTEGER as actividades_hoy,
        COUNT(CASE WHEN p.fecha > CURRENT_DATE THEN 1 END)::INTEGER as actividades_futuras,
        COUNT(CASE WHEN p.fecha < CURRENT_DATE THEN 1 END)::INTEGER as actividades_pasadas
    FROM premiacion p
    INNER JOIN premiacion_evento pe ON p.premiacion_id = pe.fk_premiacion
    WHERE pe.fk_evento = evento_id_param
    AND p.tipo = 'actividad';
