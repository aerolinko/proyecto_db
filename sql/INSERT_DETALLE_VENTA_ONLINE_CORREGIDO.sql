-- =====================================================
-- INSERT DETALLE VENTA ONLINE CORREGIDO
-- =====================================================
...
-- 2. Verificar qué productos existen en almacén (sin especificar columna de precio)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM almacen_cerveza LIMIT 1) THEN
        ...
        -- Crear almacen_cerveza (sin especificar columna de precio)
        INSERT INTO almacen_cerveza (fk_cerveza_presentacion, cantidad_stock) VALUES
        (cp_id_val, 100)
        ON CONFLICT DO NOTHING
...
-- 5. Crear detalles de venta online con precio fijo
DO $$
DECLARE
...
BEGIN
    ...
    SELECT almacen_cerveza_id INTO almacen_id_val FROM almacen_cerveza LIMIT 1;
    
    IF venta_id_val IS NOT NULL AND almacen_id_val IS NOT NULL THEN
        -- Crear detalle de venta online con precio fijo
        INSERT INTO detalle_venta_online (fk_venta_online, fk_almacen_cerveza, precio_unitario, cantidad) VALUES
        (venta_id_val, almacen_id_val, 50000, 2)
        ON CONFLICT DO NOTHING;
        
        RAISE NOTICE 'Detalle de venta creado: venta_id=%, almacen_id=%, precio=50000, cantidad=2', 
                    venta_id_val, almacen_id_val;
    ELSE
